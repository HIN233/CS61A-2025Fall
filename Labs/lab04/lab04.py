...
def buy(fruits_to_buy: list[str], prices: dict[str, int], total_amount: int) -> None:
    ...
    def add(fruits: list[str], amount: int, cart: str) -> None:
        if fruits == [] and amount == 0:
            ...
            fruit = fruits[0]
            price = prices[fruit]
            for k in range(1, total_amount // price):
...
def distance(city_a, city_b):
    ...
    5.0
    """
    len_a = get_lat(city_a)
    lon_a = get_lon(city_a)
    len_b = get_lat(city_b)
    lon_b = get_lon(city_b)
    return sqrt((len_a - len_b) ** 2 + (lon_a - lon_b) ** 2)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    dis_a = distance(point,city_a)
    dis_b = distance(point, city_b)
